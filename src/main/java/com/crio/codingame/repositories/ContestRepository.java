...
public class ContestRepository implements IContestRepository {
    ...
    public List<Contest> findAll() {
        List<Contest> allContests = contestMap.values().stream().map(contest -> new Contest(contest.getId(), contest.getName(), contest.getQuestions(), contest.getLevel(), contest.getCreator(), contest.getContestStatus())).collect(Collectors.toList());
        return allContests;
    }
    ...
    public List<Contest> findAllContestLevelWise(Level level) {
        List<Contest> levelWiseContests = contestMap.values().stream()
                                                                .filter(contest -> contest.getLevel().equals(level))
                                                                .collect(Collectors.toList());
        return levelWiseContests;
    }
